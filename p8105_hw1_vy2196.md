...
`as.numeric` works for both `log_var` and `fact_var` where it turned
`True` and `False` into `1` and `0`, and gives the underlying integer
codes of the factor levels. However, it still didn’t work for `char_var`
because characters cannot be converted to numeric values, so R returned
NA.
